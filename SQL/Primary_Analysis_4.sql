WITH city_cancellations AS (
    SELECT
        ...
        SUM(CASE WHEN MONTH(F.order_timestamp) BETWEEN 1 AND 5 AND F.is_cancelled = 'Y' THEN 1 ELSE 0 END) AS cancelled_pre_crisis,
        SUM(CASE WHEN MONTH(F.order_timestamp) BETWEEN 6 AND 9 THEN 1 ELSE 0 END) AS total_crisis_orders,
        SUM(CASE WHEN MONTH(F.order_timestamp) BETWEEN 6 AND 9 AND F.is_cancelled = 'Y' THEN 1 ELSE 0 END) AS cancelled_crisis
